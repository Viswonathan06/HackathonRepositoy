...
def getResponse():
    ...
    results = AIOrNot(str(directory))
    response["analysis"]["voiceType"]["type"] = "Human" if results['human_detected'][0] else 'AI'
    response["analysis"]["voiceType"]["probability"]["human"] = results['confidence_scores'][0]
    response["analysis"]["voiceType"]["probability"]["ai"] = results['confidence_scores'][0]
